...
export async function start(opts: ModeBootOptions): Promise<ModeController> {
  ...
  const mFps = $("m-fps");
  const mEar = $("m-ear");
  const mPitch = $("m-pitch");
  const mYaw = $("m-yaw");
  const mRoll = $("m-roll");
  const mClosed = $("m-closed");
  const mBent = $("m-bent");
  const mEye = $("m-eye");
  const mHead = $("m-head");
  ...
  const PARAM_CONTROLS: Record<keyof DetectorConfig, ParamControl> = {
    earThreshold: {
      slider: $("s-ear") as HTMLInputElement,
      value: $("v-ear") as HTMLSpanElement,
    ...
    drowsyFrames: {
      slider: $("s-drowsy-frames") as HTMLInputElement,
      value: $("v-drowsy-frames") as HTMLSpanElement,
      format: (n) => `${n}`,
      apply: (n) => {
    ...
    pitchThresholdDeg: {
      slider: $("s-pitch") as HTMLInputElement,
      value: $("v-pitch") as HTMLSpanElement,
    ...
    rollThresholdDeg: {
      slider: $("s-roll") as HTMLInputElement,
      value: $("v-roll") as HTMLSpanElement,
      format: (n) => `${n.toFixed(0)}°`,
      apply: (n) => {
    ...
    bentFrames: {
      slider: $("s-bent-frames") as HTMLInputElement,
      value: $("v-bent-frames") as HTMLSpanElement,
      format: (n) => `${n}`,
      apply: (n) => {
